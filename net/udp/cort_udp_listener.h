#ifndef CORT_UDP_CONNECT_LISTEN_ACCEPT_H_
#define CORT_UDP_CONNECT_LISTEN_ACCEPT_H_
...
struct udp_ctrler_static_creator{
	typedef udp_ctrler_static_creator<connection_t, connection_waiter_t> this_type;
	static void create(int fd, int dest_ip, int dest_port, cort_udp_connection_waiter* waiter, uint32_t init_poll_result){
...
struct cort_udp_listener : public cort_fd_waiter{
public:
	CO_DECL(cort_udp_listener)
	...
	void set_ctrler_creator(){
		ctrler_creator = udp_ctrler_static_creator<accept_cort_type, connection_waiter_t>::create;
	}
	void set_ctrler_creator(void (*ctrler_creator_arg)(int accept_fd, int dest_ip, int dest_port, cort_udp_connection_waiter* waiter, uint32_t init_poll_result)){
...
private:
	...
	void (*ctrler_creator)(int accept_fd, int dest_ip, int dest_port, cort_udp_connection_waiter* waiter, uint32_t init_poll_result);
	int backlog;
...
struct cort_udp_server_waiter : public cort_udp_connection_waiter{
	cort_udp_server_waiter(int fd){
		set_cort_fd(fd);
	}
	void (*ctrler_creator)(int accept_fd, int dest_ip, int dest_port, cort_udp_connection_waiter* waiter, uint32_t init_poll_result);
